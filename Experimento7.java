...
public class Experimento7 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            output_files.add(new PrintStream(new FileOutputStream("exp7_" + "HC_" + nameHeuristic + "_RH" + "_.txt", false)));
            ...
            for (int n_map = 0; n_map<num_map_seeds; ++n_map) {
                ...
                for (int n_rep = 0; n_rep<num_init_reps; ++n_rep) {
                    ...
                    for (int nfurgos = 5; nfurgos < nmaxfurgos; nfurgos += 5) {
                        System.setOut(output_files.get(demandType));

                        String[] main_args_rush = {"--rformat-no-tags","-r", Integer.toString(repetitions), "-m", Integer.toString(map_seed), "-i", Integer.toString(init_seed), "-rh", "-he", Integer.toString(h), "-num_furgos", Integer.toString(nfurgos)};
                        String[] main_args_equilibrium = {"--rformat-no-tags","-r", Integer.toString(repetitions), "-m", Integer.toString(map_seed), "-i", Integer.toString(init_seed), "-he", Integer.toString(h), "-num_furgos", Integer.toString(nfurgos)};
                        String[] main_args;
